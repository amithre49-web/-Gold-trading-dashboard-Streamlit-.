...
def download_15m(ticker="GC=F", period="60d"):
    ...
    Returns (df, used_ticker, used_period) where df is the first valid OHLCV DataFrame found.
    """
    candidates = []
        ...
        for t in candidates:
            ...
            if isinstance(df, pd.DataFrame) and not df.empty:
                ...
                if all(c in df.columns for c in ["Open", "High", "Low", "Close", "Volume"]):
                    return df[["Open", "High", "Low", "Close", "Volume"]], t, p
                else:
...
# ---------------- Data download ----------------n
df15, used_ticker, used_period = download_15m(ticker=ticker, period=period)
if df15.empty:
    st.error("No data returned from Yahoo Finance. Try a different ticker (GC=F) or shorter period.")
    st.stop()

# show which ticker/period succeeded
if used_ticker and used_period:
    st.success(f"Loaded data from {used_ticker} with period {used_period}")
